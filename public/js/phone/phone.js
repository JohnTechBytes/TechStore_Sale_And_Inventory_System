...
$(document).on('click', '.deleteUserBtn', function () {
    ...
    if (confirm('Are you sure you want to delete this user?')) {
        $.ajax({
            url: baseUrl + 'phone/delete/' + userId,
            method: 'POST', 
            data: {
                _method: 'DELETE',
                [csrfName]: csrfToken
            ...
            success: function (response) {
                if (response.success) {
                    showToast('success', 'phone deleted successfully.');
                    setTimeout(() => location.reload(), 1000);
                } else {
...
$(document).ready(function () {
    ...
    $table.DataTable({
        ...
        serverSide: true,
        ajax: {
            url: baseUrl + 'phone/fetchRecords',
            type: 'POST',
            headers: {
